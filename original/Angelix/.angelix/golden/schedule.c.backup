...
finish_process()
{
    ...
    if (cur_proc)
    {
        new_val = cur_proc->val;
        printf("%d\n", ANGELIX_OUTPUT(int, new_val, "output"));
        fprintf(debug_fp, "output: %d\n", new_val); fflush(debug_fp); /* DEBUG */
        free_ele(cur_proc);
        num_processes--;
    }
}
...
char *argv[];
{
    ...
    debug_fp = fopen("/tmp/golden", "w"); /* DEBUG */

    if (argc < (MAXPRIO+2))
    {
      fprintf(debug_fp, "output: %d\n", INCORRECT_USAGE); fflush(debug_fp); /* DEBUG */
    ...
    for (fseek(stdin, file_pointer, SEEK_SET), status = fscanf(in_fp, "%d", &command), file_pointer = ftell(stdin);
         ((status!=EOF) && status);
         fseek(stdin, file_pointer, SEEK_SET), status = fscanf(in_fp, "%d", &command), file_pointer = ftell(stdin))
      {
        ...
        switch(command)
          ...
          case QUANTUM_EXPIRE:
            ...
            break;
          case UNBLOCK:
            fseek(stdin, file_pointer, SEEK_SET);
            fscanf(in_fp, "%f", &ratio);
            file_pointer = ftell(stdin);
            unblock_process(ratio);
            break;
          case UPGRADE_PRIO:
            fseek(stdin, file_pointer, SEEK_SET);
            ...
            if (prio > MAXPRIO || prio <= 0) {
              fprintf(debug_fp, "output: %d\n", INVALID_PRIORITY); fflush(debug_fp); /* DEBUG */

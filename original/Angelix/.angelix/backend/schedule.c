...
float ratio;
{
    ...
    if (count > 0)
    {
	upgrade = ANGELIX_CHOOSE(int, 1, 221, 12, 221, 12, ((char*[]){"upgrade", "count"}), ((int[]){upgrade, count}), 2); n = (int) (count*ratio + 1);
	if(ratio == 1.0) n--; /* Correct original */
	proc = find_nth(src_queue, n);
	if (ANGELIX_CHOOSE(bool, proc, 224, 6, 224, 6, ((char*[]){"proc", "count", "n", "upgrade"}), ((int[]){proc, count, n, upgrade}), 4)) {

...
char *argv[];
{
    ...
    {
	nprocs = atoi(argv[MAXPRIO + 1 - prio]); if(nprocs < 0) exit_here(BADARG);
	for(; nprocs > 0; nprocs--)
	{
	    status = new_job(prio); if(status) exit_here(status);
	}
    }
     ...
     int prio;
{
    int pid, status = OK;
    struct process *new_process;
    if(prio < 1) return(BADPRIO); /* FIX: if(0) */
    pid = next_pid++;
     ...
     float ratio;
{
    int status;
    struct process * job;
    status = get_process(BLOCKPRIO, ratio, &job); if(status <= 0) return(status);
...
finish() /* Get current job, print it, and zap it. */
{
	...
	reschedule(0);
	printf("%d\n", ANGELIX_OUTPUT(int, job->pid, "output"));
	free(job);
	return(FALSE);
    }
    else return(TRUE);

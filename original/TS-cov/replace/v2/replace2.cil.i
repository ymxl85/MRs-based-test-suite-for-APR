# 1 "coverageTC/v2/replace2.cil.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "coverageTC/v2/replace2.cil.c"
...
void dodash(char delim , char *src , int *i , char *dest , int *j , int maxset )
{
  ...
  while ((int )*(src + *i) != (int )delim && (int )*(src + *i) != 0) {
    if ((int )*(src + *i) != 45) {
      tmp = addstr(*(src + *i), dest, j, maxset);
      junk = (bool )tmp;
    } else
    if (*j <= 1 || (int )*(src + (*i + 1)) == 0) {
      tmp___0 = addstr((char )'-', dest, j, maxset);
      junk = (bool )tmp___0;
    } else {
      tmp___3 = isalnum((int )*(src + (*i - 1)));
      if (tmp___3) {
        ...
        if (tmp___4) {
          if ((int )*(src + (*i - 1)) <= (int )*(src + (*i + 1))) {
            k = (int )*(src + (*i - 1)) + 1;
            while (k <= (int )*(src + (*i + 1))) {
              tmp___1 = addstr((char )k, dest, j, maxset);
              junk = (bool )tmp___1;
              k ++;
            }
            (*i) ++;
          } else {
            tmp___2 = addstr((char )'-', dest, j, maxset);
            junk = (bool )tmp___2;
          }
        } else {
          tmp___2 = addstr((char )'-', dest, j, maxset);

...
{ 
  ...
  if (_coverage_fout == 0) {
    {
    _coverage_fout = fopen("/home/mingyue/experiments/GenProg/genprog-source-v3.0/data/tcas/coverageTC/v31/./coverage.path",
                           "wb");
    }
  }
  }
  ...
  if (upward_preferred) {
    ...
    if (tmp___5) {
      ...
      if (Cur_Vertical_Sep >= 300) {
        ...
        tmp___6 = ALIM(Cur_Vertical_Sep, High_Confidence,
                       Two_of_Three_Reports_Valid, Own_Tracked_Alt,
                       Own_Tracked_Alt_Rate, Other_Tracked_Alt, Alt_Layer_Value,
                       Up_Separation, Down_Separation, Other_RAC,
                       Other_Capability, Climb_Inhibit);

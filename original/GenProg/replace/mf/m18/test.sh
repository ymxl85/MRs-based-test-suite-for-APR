...
' | diff outputP/O11 - && exit 0 ;;
  p12) $1 '?'	'b'	'
u' | diff outputP/O12 - && exit 0 ;;
  p13) $1 'À'	'b'	'
u' | diff outputP/O13 - && exit 0 ;;
  p14) $1 '?ÿ'	'b'	'
...
' | diff outputP/O21 - && exit 0 ;;
  p22) $1 '[À]'	'b'	'
u' | diff outputP/O22 - && exit 0 ;;
  p23) $1 '[^]ÿ'	'b'	'
u' | diff outputP/O23 - && exit 0 ;;
  p24) $1 '[À]ÿ'	'b'	'
u' | diff outputP/O24 - && exit 0 ;;
  n1) $1 '[^]'	'd'	'
...
' | diff outputP/O2 - && let fit=$fit+1
  $1 '?'	'a'	'
u' | diff outputP/O3 - && let fit=$fit+1
  $1 '?'	'b'	'
...
' | diff outputP/O11 - && let fit=$fit+1
  $1 '?'	'b'	'
u' | diff outputP/O12 - && let fit=$fit+1
  $1 'À'	'b'	'
u' | diff outputP/O13 - && let fit=$fit+1
  $1 '?ÿ'	'b'	'
...
' | diff outputP/O21 - && let fit=$fit+1
  $1 '[À]'	'b'	'
u' | diff outputP/O22 - && let fit=$fit+1
  $1 '[^]ÿ'	'b'	'
u' | diff outputP/O23 - && let fit=$fit+1
  $1 '[À]ÿ'	'b'	'
u' | diff outputP/O24 - && let fit=$fit+1
  $1 '[^]'	'd'	'
...
' | diff outputF/O2 - && let fit=$fit+1
  $1 '[^]'	'a'	'
u' | diff outputF/O3 - && let fit=$fit+1
  $1 '[^]'	'b'	'
...
' | diff outputF/O5 - && let fit=$fit+1
  $1 '[^]'	'b'	'
u' | diff outputF/O6 - && let fit=$fit+1

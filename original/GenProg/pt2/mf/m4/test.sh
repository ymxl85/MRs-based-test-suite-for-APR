...
ß ' | diff outputF/O14 - && exit 0 ;;
 ...
 ß' | diff outputF/O15 - && exit 0 ;;
  n16) $1 ' x; 9õ  ' | diff outputF/O16 - && exit 0 ;;
  n17) $1 'ßx; "
  ' | diff outputF/O17 - && exit 0 ;;
  n18) $1 ' {; "
  ' | diff outputF/O18 - && exit 0 ;;
  n19) $1 ' x; ]
...
ß ' | diff outputF/O20 - && exit 0 ;;
 ...
 ß' | diff outputF/O21 - && exit 0 ;;
  n22) $1 ' x; "õ  ' | diff outputF/O22 - && exit 0 ;;
  n23) $1 'ßx; x
  ' | diff outputF/O23 - && exit 0 ;;
  n24) $1 ' {; x
  ' | diff outputF/O24 - && exit 0 ;;
  n25) $1 ' x; {
...
ß ' | diff outputF/O26 - && exit 0 ;;
 ...
 ß' | diff outputF/O27 - && exit 0 ;;
  n28) $1 ' x; xõ  ' | diff outputF/O28 - && exit 0 ;;
...
ß ' | diff outputP/O15 - && let fit=$fit+1
  $1 '  
 ß' | diff outputP/O16 - && let fit=$fit+1
  $1 'ß 
...
ß ' | diff outputP/O21 - && let fit=$fit+1
  $1 ' x
 ß' | diff outputP/O22 - && let fit=$fit+1
  $1 '  ; "
...
ß ' | diff outputF/O6 - && let fit=$fit+1
 ...
 ß' | diff outputF/O7 - && let fit=$fit+1
  $1 '  ; "õ  ' | diff outputF/O8 - && let fit=$fit+1
  $1 'ß ; "
...
ß ' | diff outputF/O14 - && let fit=$fit+1
 ...
 ß' | diff outputF/O15 - && let fit=$fit+1
  $1 ' x; 9õ  ' | diff outputF/O16 - && let fit=$fit+1
  $1 'ßx; "
  ' | diff outputF/O17 - && let fit=$fit+1
  $1 ' {; "
  ' | diff outputF/O18 - && let fit=$fit+1
  $1 ' x; ]
  ' | diff outputF/O19 - && let fit=$fit+1
  $1 ' x; "
ß ' | diff outputF/O20 - && let fit=$fit+1
  $1 ' x; "
 ß' | diff outputF/O21 - && let fit=$fit+1
  $1 ' x; "õ  ' | diff outputF/O22 - && let fit=$fit+1
  $1 'ßx; x
  ' | diff outputF/O23 - && let fit=$fit+1
  $1 ' {; x
  ' | diff outputF/O24 - && let fit=$fit+1
  $1 ' x; {
  ' | diff outputF/O25 - && let fit=$fit+1
  $1 ' x; x
ß ' | diff outputF/O26 - && let fit=$fit+1
  $1 ' x; x
 ß' | diff outputF/O27 - && let fit=$fit+1
  $1 ' x; xõ  ' | diff outputF/O28 - && let fit=$fit+1

...
;"' | diff outputP/O28 - && exit 0 ;;
  p29) $1 '  
"Ý' | diff outputP/O29 - && exit 0 ;;
  p30) $1 'ß 
""' | diff outputP/O30 - && exit 0 ;;
  p31) $1 '  
"' | diff outputP/O31 - && exit 0 ;;
  p32) $1 'ß 
"}' | diff outputP/O32 - && exit 0 ;;
  p33) $1 ' ß
"}' | diff outputP/O33 - && exit 0 ;;
  p34) $1 '  
...
ß"' | diff outputF/O11 - && exit 0 ;;
 ...
 ;' | diff outputF/O12 - && exit 0 ;;
  n13) $1 '  ; "õ "' | diff outputF/O13 - && exit 0 ;;
  n14) $1 'ß ; "
...
;"' | diff outputF/O16 - && exit 0 ;;
  n17) $1 '  ; "
"Ý' | diff outputF/O17 - && exit 0 ;;
  n18) $1 '  ; "õ""' | diff outputF/O18 - && exit 0 ;;
  n19) $1 'ß ; "
""' | diff outputF/O19 - && exit 0 ;;
  n20) $1 '  ; "
...
"}' | diff outputF/O22 - && exit 0 ;;
  n23) $1 '  ; "
;}' | diff outputF/O23 - && exit 0 ;;
  n24) $1 '  ; "õ"}' | diff outputF/O24 - && exit 0 ;;
  n25) $1 '  ; "
"
' | diff outputF/O25 - && exit 0 ;;
...
"}' | diff outputP/O18 - && let fit=$fit+1
  $1 '  
ß ' | diff outputP/O19 - && let fit=$fit+1
  $1 '  
...
;"' | diff outputP/O28 - && let fit=$fit+1
  $1 '  
"Ý' | diff outputP/O29 - && let fit=$fit+1
  $1 'ß 
""' | diff outputP/O30 - && let fit=$fit+1
  $1 '  
"' | diff outputP/O31 - && let fit=$fit+1
  $1 'ß 
"}' | diff outputP/O32 - && let fit=$fit+1
  $1 ' ß
"}' | diff outputP/O33 - && let fit=$fit+1
  $1 '  
...
"}' | diff outputF/O4 - && let fit=$fit+1
  $1 '  ; "
ß ' | diff outputF/O5 - && let fit=$fit+1
  $1 '  ; "
 ß' | diff outputF/O6 - && let fit=$fit+1
  $1 '  ; "õ  ' | diff outputF/O7 - && let fit=$fit+1
  $1 'ß ; "
 ...
 "' | diff outputF/O10 - && let fit=$fit+1
  $1 '  ; "
ß"' | diff outputF/O11 - && let fit=$fit+1
  $1 '  ; "
 ;' | diff outputF/O12 - && let fit=$fit+1
  $1 '  ; "õ "' | diff outputF/O13 - && let fit=$fit+1
  $1 'ß ; "
...
;"' | diff outputF/O16 - && let fit=$fit+1
  $1 '  ; "
"Ý' | diff outputF/O17 - && let fit=$fit+1
  $1 '  ; "õ""' | diff outputF/O18 - && let fit=$fit+1
  $1 'ß ; "
""' | diff outputF/O19 - && let fit=$fit+1
  $1 '  ; "
...
"}' | diff outputF/O22 - && let fit=$fit+1
  $1 '  ; "
;}' | diff outputF/O23 - && let fit=$fit+1
  $1 '  ; "õ"}' | diff outputF/O24 - && let fit=$fit+1
  $1 '  ; "
"
' | diff outputF/O25 - && let fit=$fit+1
  let passed_all_so_stop_search="$fit >=60 "
  echo $fit > $5

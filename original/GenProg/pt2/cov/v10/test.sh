...
case $2 in  p1) $1 '\n' | diff outputP/O1 - && exit 0 ;;
  ...
  p12) $1 'xx' | diff outputP/O12 - && exit 0 ;;
  p13) $1 '""w' | diff outputP/O13 - && exit 0 ;;
  p14) $1 'x_' | diff outputP/O14 - && exit 0 ;;
  p15) $1 '""9' | diff outputP/O15 - && exit 0 ;;
  p16) $1 'x9' | diff outputP/O16 - && exit 0 ;;
  p17) $1 '97' | diff outputP/O17 - && exit 0 ;;
  p18) $1 '[\xd5' | diff outputP/O18 - && exit 0 ;;
  p19) $1 '#{' | diff outputP/O19 - && exit 0 ;;
  p20) $1 '#x' | diff outputP/O20 - && exit 0 ;;
  p21) $1 '""[' | diff outputP/O21 - && exit 0 ;;
...
let fit=0
  ...
  $1 'xx' | diff outputP/O12 - && let fit=$fit+1
  $1 '""w' | diff outputP/O13 - && let fit=$fit+1
  $1 'x_' | diff outputP/O14 - && let fit=$fit+1
  $1 '""9' | diff outputP/O15 - && let fit=$fit+1
  $1 'x9' | diff outputP/O16 - && let fit=$fit+1
  $1 '97' | diff outputP/O17 - && let fit=$fit+1
  $1 '[\xd5' | diff outputP/O18 - && let fit=$fit+1
  $1 '#{' | diff outputP/O19 - && let fit=$fit+1
  $1 '#x' | diff outputP/O20 - && let fit=$fit+1
  $1 '""[' | diff outputP/O21 - && let fit=$fit+1

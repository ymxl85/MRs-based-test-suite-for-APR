...
{ int status ;
  ...
  {
  __repair_swap1_146__c2: /* CIL Label */ 
  __repair_swap1_144__c1: /* CIL Label */ 
  return (tmp);
  if (status <= 0) {
    return (status);
  } else {
  ...
  __repair_swap1_142__c3: /* CIL Label */ 
  status = get_process(prio, ratio, & job);
  tmp = enqueue(prio + 1, job);
  __repair_swap1_142__c4: /* CIL Label */ 
  status = get_process(prio, ratio, & job);
}
}
int block(void) 
...
{ struct process *job ;
  ...
  {
  s[0] = (char )'\000';
  __repair_del_176__c5: /* CIL Label */ 
  {

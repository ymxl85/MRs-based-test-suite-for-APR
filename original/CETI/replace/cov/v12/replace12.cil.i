# 1 "coverageTC/v12/replace12.cil.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "coverageTC/v12/replace12.cil.c"
...
bool getccl(char *arg , int *i , char *pat , int *j )
{
  ...
  (*i) ++;
  if ((int )*(arg + *i) == 94) {
    tmp = addstr((char )'!', pat, j, (int )(& MAXPAT));
    junk = (bool )tmp;
    (*i) ++;
  } else {
    tmp___0 = addstr((char )'[', pat, j, (int )(& MAXPAT));
    junk = (bool )tmp___0;
  }
  jstart = *j;
  tmp___1 = addstr((char)0, pat, j, (int )(& MAXPAT));
  junk = (bool )tmp___1;
  dodash((char )']', arg, i, pat, j, (int )(& MAXPAT));
...
void stclose(char *pat , int *j , int lastj )
{
  ...
  while (jp >= lastj) {
    jt = jp + 1;
    tmp = addstr(*(pat + jp), pat, & jt, (int )(& MAXPAT));
    junk = (bool )tmp;
    jp --;
  }
  (*j) ++;
...
int makepat(char *arg , int start , char delim , char *pat )
{
  ...
  while ((! done && (int )*(arg + i) != (int )delim) && (int )*(arg + i) != 0) {
    ...
    if ((int )*(arg + i) == 36 && (int )*(arg + (i + 1)) == (int )delim) {
      tmp___1 = addstr((char )'$', pat, & j, (int )(& MAXPAT));
      junk = (bool )tmp___1;
    } else
    if ((int )*(arg + i) == 91) {
    ...
    if ((int )*(arg + i) == 42 && i > start) {
      ...
      tmp___3 = addstr((char )'c', pat, & j, (int )(& MAXPAT));
      junk = (bool )tmp___3;
      escjunk = esc(arg, & i);
      tmp___4 = addstr(escjunk, pat, & j, (int )(& MAXPAT));
      junk = (bool )tmp___4;
    }
    lastj = lj;
    if (! done) {
  ...
  tmp___5 = addstr((char )'\000', pat, & j, (int )(& MAXPAT));
  junk = (bool )tmp___5;
...
int makesub(char *arg , int from , character delim , char *sub )
{
  ...
  while ((int )*(arg + i) != (int )delim && (int )*(arg + i) != 0) {
    if ((unsigned int )*(arg + i) == 38U) {
      tmp = addstr((char)-1, sub, & j, (int )(& MAXPAT));
      junk = (bool )tmp;
    } else {
      escjunk = esc(arg, & i);
      tmp___0 = addstr(escjunk, sub, & j, (int )(& MAXPAT));
      junk = (bool )tmp___0;
    }
    i ++;
  }
  if ((int )*(arg + i) != (int )delim) {
    ...
    tmp___1 = addstr((char )'\000', sub, & j, (int )(& MAXPAT));
    junk = (bool )tmp___1;
    if (! junk) {
      result = 0;
    } else {

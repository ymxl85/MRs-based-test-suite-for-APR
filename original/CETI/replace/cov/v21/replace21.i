...
# 5 "coverageTC/v21/replace21.c" 2

void Caseerror();

typedef char bool;
# 35 "coverageTC/v21/replace21.c"
...
int *j;
{
    ...
    *i = *i + 1;
    if (arg[*i] == '^') {
 junk = addstr('!', pat, j, 99);
 *i = *i + 1;
    } else
 junk = addstr('[', pat, j, 99);
    jstart = *j;
    junk = addstr(0, pat, j, 99);
    dodash(']', arg, i, pat, j, 99);
...
char *pat;
{
 ...
 {
     junk = addstr('c', pat, &j, 99);
     escjunk = esc(arg, &i);
     junk = addstr(escjunk, pat, &j, 99);
 }
 lastj = lj;
 if ((!done))
     i = i + 1;
 ...
 char* sub;
{
 ...
 if ((arg[i] == (unsigned)('&')))
     junk = addstr(-1, sub, &j, 99);
 else {
     escjunk = esc(arg, &i);
     junk = addstr(escjunk, sub, &j, 99);
 }
 i = i + 1;
    }
    if (arg[i] != delim)

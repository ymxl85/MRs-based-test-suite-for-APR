...
type inp_t = int list 
type outp_t = int 
type float_t = float list (*jmy add*)
type testcase_t = float_t*outp_t (*jmy:inp_t->float_t*)
type sid_t = int
...
let mk_testscript (testscript:string) (tcs:(float list * int) list) = (*jmy:int->float*)
  (*"($1 1071 1029 | diff ref/output.1071.1029 - && (echo "1071 1029" >> $2)) &"*)

  let content = L.map (fun (inp,_) ->
    let inp = String.concat " " (L.map string_of_float inp) in (*jmy int->float*)
...
let string_of_tc (tc:float_t * outp_t) : string =  (*jmy inp_t->float_t*)
  let inp,outp = tc in 
  let inp = String.concat "; " (L.map string_of_float inp) in (*jmy:int->float t*)
...
class uTest (filename:string) = object(self)
  ...
  method get_tcs (inputs:string) (outputs:string) = 
	...
	(try
	   let inp = L.map float_of_string inp in (*jmy:int->float*)
...
let mk_main (main_fd:fundec) (mainQ_fd:fundec) (tcs:testcase_t list) 
  ...
  let rs = L.map (fun (inps, outp) -> 
    ...
    let args = L.map integer inps in (*jmy:integer->float*)

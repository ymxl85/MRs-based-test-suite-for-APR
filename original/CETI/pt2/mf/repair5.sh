...
echo "***************"$path
cd ..
cilly $path/pt$v.c --save-temps=$path --noPrintLn --useLogicalOperators
rm -rf /tmp/cece_ &>t
time ./tf $path/pt$v.cil.i $path/Outer.inputs $path/Inner.inputs $path/outputs/ --tcnumber $tcn --fl_sids "131" --no_parallel --top_n_sids 80 --debug --min_sscore 0.01
#--fl_sids "8" --debug 

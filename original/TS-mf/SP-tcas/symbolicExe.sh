...
#klee --allow-external-sym-calls  --write-cvcs --optimize --max-time=10 $current-s.o >> out.log--only-output-states-covering-new
klee --write-paths --allow-external-sym-calls --optimize $current-s.o > out.log
...
klee --write-paths --allow-external-sym-calls --optimize --only-output-states-covering-new  $current-f.o > out.log
 cp out.log klee-out-0/
java OutputAnalyzer3 klee-out-0/
  cd -
  #cd ..

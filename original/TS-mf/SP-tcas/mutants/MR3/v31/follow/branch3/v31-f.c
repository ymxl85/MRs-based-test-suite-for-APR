  ...
  if (upward_preferred)
    {
      ...
      result = result && (Own_Tracked_Alt <= Other_Tracked_Alt); //tvn 1/4 changes (need remove)
    }
  else
    {	
      ...
      result = result && (Own_Tracked_Alt < Other_Tracked_Alt); //tvn 2/4 change  (need removal)
    }
  return result;
}

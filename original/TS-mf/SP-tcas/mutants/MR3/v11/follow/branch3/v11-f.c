...
int Own_Below_Threat(int Cur_Vertical_Sep, 
		     ...
		     int Climb_Inhibit)
{
  return (Own_Tracked_Alt <= Other_Tracked_Alt); /*tvn: operator change < (hard, requires 2 changes)*/
}
...
int Own_Above_Threat(int Cur_Vertical_Sep, 
		     ...
		     int Climb_Inhibit)
{
  return (Other_Tracked_Alt <= Own_Tracked_Alt); /*tvn: operator change < (hard, requires 2 changes)*/
}
	...
	Own_Below_Threat(Cur_Vertical_Sep, 
      ...
      need_downward_RA = Non_Crossing_Biased_Descend(Cur_Vertical_Sep, 
						     ...
						     Climb_Inhibit) && Own_Above_Threat(Cur_Vertical_Sep, 
											...
											Other_Capability,
											Climb_Inhibit);
      if (need_upward_RA) //tvn: remove condition

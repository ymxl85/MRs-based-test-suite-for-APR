...
do
  ...
  #select the corresponding branch
    ...
    do
      ...
      #x1=$(cat $sOut)
      #x2=$(cat ./mutants/$MR/$current/precondition$i)
      #if [ $x1 = $x2 ]
      then
        continue
      else
  ...
  epOut=./mutants/$MR/expectedOutput
  for fPC in ./mutants/$MR/$current/follow/branch$i/klee-out-0/test*.cvc
  do
    fOut=$fPC.out
    if [ -e $fOut ]
    then
      diff $epOut $fOut > ./mutants/$MR/$current/diffout.txt
      #os=$(cat $epOut)
      #of=$(cat $fOut)
      if [ -s ./mutants/$MR/$current/diffout.txt ] #output is different from the expected output
      #if [ $os = $of ]
      #then
      #  continue
      then #their outputs violate the MR, which requires the equality between these two outputs.

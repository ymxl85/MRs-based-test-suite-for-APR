...
char	*pat;
{
	...
	else if ((arg[i] == EOL) /*&& (arg[i+1] == delim fault*/)
...
void
change(pat, sub,line,r)
char *pat, *sub, *line,*r;
{
  subline(line, pat, sub,r);
}
void mainQ(char *argv1, char *argv2, char *argv3,char *r)
{
   string pat, sub;
   bool result;
   result = getpat(argv1, pat);
   result = getsub(argv2, sub);
   change(pat,sub,argv3,r);
}
//a2 matches, a1 doesn't: a1s
int main()
{
  char argv1[3];
   char argv2[2];
   ...
   klee_make_symbolic(&argv3,sizeof(argv3),"argv3");
    ...
    klee_assume(argv1[1]=='\0');
    //ts.substitution=s
    klee_assume(argv2[0]>='0' & argv2[0]<='9');
    klee_assume(argv2[1]=='\0');
    //ts.text=a1a2
    klee_assume(argv3[0]>0 & argv3[0]<128 & argv3[0]!='$' & argv3[0]!='[' & argv3[0]!=']' & argv3[0]!='*' & argv3[0]!='?' & argv3[0]!=92 & argv3[0]!='Z' & argv3[0]!='-' & argv3[0]!=',');
    klee_assume(argv3[1]>0 & argv3[1]<128 & argv3[1]!='$' & argv3[1]!='[' & argv3[1]!=']' & argv3[1]!='*' & argv3[1]!='?' & argv3[1]!=92 & argv3[1]!='Z' & argv3[1]!='-' & argv3[1]!=',');

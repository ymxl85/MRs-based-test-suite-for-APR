...
{ int command ;
  ...
  while (a3 > 0) {
    status = new_job(prio);
    __repair_app_47__517: /* CIL Label */ 
    {
    if (status) {
      exit_here(status);
    } else {
...
{ int status ;
  ...
  if ((int )*(argv + *pos) != 0) {
    tmp = -1;
    __repair_swap1_79__518: /* CIL Label */ 
    *prio = atoi(value);
    *prio = tmp;
    *ratio = (float )(- 1.0);
    getAdata(argv, pos, value);
    *command = atoi(value);
    ...
    case 1: 
    getAdata(argv, pos, value);
    __repair_swap1_95__519: /* CIL Label */ 
    *command = tmp;
    break;
    case 4: 
    getAdata(argv, pos, value);
...
{ struct process *job ;
  ...
  {
  __repair_swap1_168__51a: /* CIL Label */ 
  return (0);
  if (job) {
    current_job = (struct process *)0;
    tmp = enqueue(job->priority, job);
  ...
  __repair_swap1_166__51b: /* CIL Label */ 
  job = get_current();
}
}

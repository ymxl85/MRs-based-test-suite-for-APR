  ...
  if (enabled) {
    if (tcas_equipped) {
      ...
      if (! tcas_equipped) {
        ...
        need_downward_RA = Non_Crossing_Biased_Descend(Cur_Vertical_Sep, High_Confidence, Two_of_Three_Reports_Valid, Own_Tracked_Alt, Own_Tracked_Alt_Rate, Other_Tracked_Alt, Alt_Layer_Value, Up_Separation, Down_Separation, Other_RAC, Other_Capability, Climb_Inhibit);
        if (need_upward_RA) {
          if (need_downward_RA) {
            alt_sep = 0;
          } else {
...
{ int tmp ;
  ...
  __repair_swap1_124__ed: /* CIL Label */ 
  tmp___5 = atoi(*(argv + 6));
  tmp___3 = atoi(*(argv + 8));
  tmp___4 = atoi(*(argv + 7));
  __repair_swap1_121__ee: /* CIL Label */ 
  tmp___2 = atoi(*(argv + 9));
  tmp___6 = atoi(*(argv + 5));
  __repair_del_126__ef: /* CIL Label */ 
  {

  }
  tmp___8 = atoi(*(argv + 3));
  __repair_del_128__f0: /* CIL Label */ 
  {

#ifndef ANGELIX_OUTPUT /* ADDED */
#define ANGELIX_OUTPUT(type, expr, id) expr /* ADDED */
#endif /* ADDED */

#define INCORRECT_USAGE  254 /* ADDED */
#define INVALID_PRIORITY 255 /* ADDED */
...
Ele *find_nth(List *f_list, int n)
{
    ...
    f_ele = f_list->first;
    for (i=1;
         f_list->first && (i<n); /* FIX: f_ele && (i<n) */
         i++) {
      if (!f_ele) exit(2);
      f_ele = f_ele->next;
    }
...
finish_process()
{
    ...
    if (cur_proc)
    {
      printf("%d\n", ANGELIX_OUTPUT(int, cur_proc->val, "output")); /* ADDED */
      free_ele(cur_proc);
      num_processes--;
    }
}
...
char *argv[];
{
    ...
    int status;
    FILE *in_fp; /* ADDED */
    long in_fp_offset = 0; /* ADDED */

    debug_fp = fopen("/tmp/debug", "w"); /* DEBUG */

    if (argc < (MAXPRIO+2)) /* MODIFIED */
    {
      printf("%d\n", ANGELIX_OUTPUT(int, INCORRECT_USAGE, "output")); /* ADDED */
      return 1; /* MODIFIED */
    }    

    in_fp = fopen(argv[1], "r"); /* ADDED */

    initialize();
    for (prio=MAXPRIO; prio >= 1; prio--)
    {
      init_prio_queue(prio, atoi(argv[prio+1])); /* MODIFIED */
    }
    fprintf(debug_fp, "ready to take in input.\n"); fflush(debug_fp); /* DEBUG */
    /* MODIFIED */
    for (fseek(in_fp, in_fp_offset, SEEK_SET), 
           status = fscanf(in_fp, "%d", &command),
           in_fp_offset = ftell(in_fp);
         ((status!=EOF) && status);
         status = fscanf(in_fp, "%d", &command))
      {
        fprintf(debug_fp, "command: %d\n", command); fflush(debug_fp); /* DEBUG */
        switch(command)
          {
          case FINISH:
            finish_process();
          ...
          case UPGRADE_PRIO:
            ...
            if (prio > MAXPRIO || prio <= 0) {
              printf("%d\n", ANGELIX_OUTPUT(int, INVALID_PRIORITY, "output")); /* ADDED */
              return 1; /* MODIFIED */
            }
            else
              upgrade_process_prio(prio, ratio);

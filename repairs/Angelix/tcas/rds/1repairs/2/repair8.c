			  ...
			  int Climb_Inhibit)
{
  ...
  return (Climb_Inhibit ? 
	  Up_Separation  //tvn bug 100 + Up_Separation
	  : Up_Separation);
			      ...
			      int Climb_Inhibit)
{
  ...
  if (upward_preferred)
    ...
    {	
////result = Own_Above_Threat(Cur_Vertical_Sep,---deleted
...
////Climb_Inhibit) && (Cur_Vertical_Sep >= 300) && (Up_Separation >= ALIM(Cur_Vertical_Sep,---deleted
////High_Confidence,---deleted
////Two_of_Three_Reports_Valid,---deleted
////Own_Tracked_Alt,---deleted
////Own_Tracked_Alt_Rate,---deleted
...
////Climb_Inhibit));---deleted
      result = (Up_Separation < sys_nerr);////---added
    }
  return result;
}
